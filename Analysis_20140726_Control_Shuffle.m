% Analysis_20140726_Control_Shuffle
% address reviewer concerns from the Science submission:
%	2)	does our analysis contain hidden circularities? Do the classification
%		analyses using shuffled labels.
nCore			= 12;
strNameAnalysis	= '20140726_control_shuffle';

strDirOut	= DirAppend(strDirAnalysis,strNameAnalysis);
CreateDirPath(strDirOut);

ifo			= GO.SubjectInfo('shuffle',true);
...
%ROI MVPA
	...
	%remake the confusion correlation plot
		h	= GO.Plot.ConfusionCorrelation(stat,...
				'outdir'	, strDirOut	, ...
				'jackknife'	, true		, ...
				'models'	, 1			, ...
				'ymin'		, -1		, ...
...
%Cross-classification
	...
	%what did we get?
		...
		p	= num2cell([fisherz(stat.shape.mRConfJK), stat.shape.tRConfJK, stat.shape.pRConfJK, stat.shape.pfdrRConfJK, fisherz(stat.operation.mRConfJK), stat.operation.tRConfJK, stat.operation.pRConfJK, stat.operation.pfdrRConfJK]);
		lbl	= {'roi1' 'roi2' 'S z(r)' 'S t' 'S p/C' 'S pFDR/C' 'O z(r)' 'O t' 'O p/C' 'O pFDR/C'};
		x	= [cMaskPairDiff p]';
		disp(sprintf(['%5s\t%5s\t' repmat('%5s\t',[1 size(p,2)])],lbl{:}));
		disp(sprintf(['%5s\t%5s\t' repmat('%.3f\t',[1 size(p,2)]) '\n'],x{:}));
...
% 						disp(sprintf('**%5s to %5s: t=%.3f p=%.4f/pC=%.4f pjk=%.4f/pjkC=%.4f pacc=%.4f/paccC=%.4f',strMaskSrc,strMaskDst,tJK,p,pfdr,pJK,pfdrJK,pacc,pfdracc));
% 					end
					disp(sprintf('%5s\t%5s\t%.3f\t%.3f\t%.3f\t%.3f',strMaskSrc,strMaskDst,zJK,tJK,pJK,pfdrJK));
				end
			end
		end
